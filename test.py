...
NEWSAPI_KEY = '2335f5f57f8242e7b970caf30d3d31b7'
OPENAI_API_KEY = ''
...
def create_main_keyboard():
    ...
    keyboard.add(types.KeyboardButton('🔍 Поиск акций'))  
...
def search_stocks(query):
    try:
        ...
        for element in all_text_elements:
            text = element.strip()
            if text:
                if text[0].isdigit() and '₽' in text and len(current_block) > 3:
                    stock_blocks.append(current_block)
                    current_block = []
                current_block.append((element.parent, text))
        ...
        for block in stock_blocks:
            block_text = " ".join(text for _, text in block).lower()
            if query in block_text:
                price = next((text for _, text in block if '₽' in text and text[0].isdigit()), None)
                if price:
                    name_parts = [text for _, text in block if text != price]
                    full_name = " ".join(name_parts[:3])
                    matches.append((full_name, price))
        
        return matches
    
    except Exception as e:
        print(f"Ошибка при поиске акций: {str(e)}")
        return None
...
def start_stock_search(message):
    ...
    bot.send_message(
        message.chat.id,
        "🔍 Введите название акции или тикер (например, Газпром или GAZP):",
...
def handle_stock_query(message):
    try:
        ...
        matches = search_stocks(message.text.strip().lower())
        
        if not matches:
            bot.send_message(
                message.chat.id,
                "❌ Акции не найдены. Попробуйте другой запрос.",
                reply_markup=create_main_keyboard()
            )
            set_state(message.chat.id, None)
            return
        ...
        response = "🔍 Найдено несколько акций:\n\n"
        for idx, (name, price) in enumerate(matches, 1):
            response += f"{idx}. {name} - {price}\n"
        
        response += "\nВведите номер акции для подробной информации:"
        ...
        bot.send_message(
            message.chat.id,
            f"⚠️ Ошибка при обработке запроса: {str(e)}",
            reply_markup=create_main_keyboard()
        )
        set_state(message.chat.id, None)
